...
function Secured() {
  ...
  useEffect(() => {
    const SecuredToken = async () => {
      const res = await fetch('https://api.growtavern.site:1515/account/secured', {
        // const res = await fetch('http://localhost:1515/account/secured', {
        method: "POST",
        headers: {
      ...
      )} */}
      {
        !Secured ? (
          ...
          <div className="flex flex-col items-center h-96 justify-center">
            <h1 className="text-2xl font-GothicExtraBold">Account Security Activated</h1>
          </div>
        )
