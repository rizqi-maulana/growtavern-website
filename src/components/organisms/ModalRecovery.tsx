...
function ModalRecovery() {
  ...
  const resend = new Resend('re_Mmgc46Ua_HjQHrUKnTosYX7Nrq9AKZctb');
      ...
      await PlayerValidate(Name, Email).then(async (res) => {
        ...
        if (resdata.type === 'success') {
          const response = await resend.emails.send({
            from: 'team@growtavern.site', // Ganti dengan alamat email pengirim
            to: ['maulanarizq26@gmail.com'], // Ganti dengan alamat email penerima
            subject: 'Pesan dengan HTML',
            html: `
              <html>
                <body>
                  <h1>Halo dari Resend!</h1>
                  <p>Ini adalah pesan email dengan format HTML.</p>
